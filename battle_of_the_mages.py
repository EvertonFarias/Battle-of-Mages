...
def criar_magias(classe):
    magias_classe = []
    if classe == 'Fogo':
        magias_classe.append(Magia('Chamas Sombrias', 100, 45))
        magias_classe.append(Magia('Chamas Espirais Profundas', 80, 40))
        magias_classe.append(Magia('Estrela Incandescente', 60, 35))

    elif classe == 'Agua':
        magias_classe.append(Magia('Projétil de Água Benta', 100, 45))
        magias_classe.append(Magia('Fúria da Serpente do Mar', 80, 40))
        magias_classe.append(Magia('Rugido do Dragão do Mar', 60, 35))

    elif classe == 'Terra':
        magias_classe.append(Magia('Obstrução de Solo', 100, 45))
        magias_classe.append(Magia('Mãe-Terra Dividida', 80, 40))
        magias_classe.append(Magia('Solo Ascendente', 60, 35))

    elif classe == 'Ar':
        magias_classe.append(Magia('Vento Cortante', 100, 45))
        magias_classe.append(Magia('Sopro do Dragão', 80, 40))
        magias_classe.append(Magia('Dança dos Ventos', 60, 35))
...
def telainicial():
    ...
    texto1_rect = texto1.get_rect(center=botao_rect.center) #start
...
def tela_vitoria():
    ...
    img = pygame.image.load("img/VITORIA.png")#Define o fundo da imagem inicial
    botao = pygame.image.load("img/botao.png")
    botao = pygame.transform.scale(botao, (240, 80))
    botao_rect = botao.get_rect()
    botao_rect.centerx = largura // 2
    botao_rect.centery = altura // 2 + 110


    fonte = pygame.font.Font(None, 27)  
    texto = fonte.render("JOGAR NOVAMENTE", True, (255, 255, 255))  
    texto_rect = texto.get_rect(center=botao_rect.center) #SERVE PARA POSICIONAR O TEXTO NO CENTRO DO BOTÃO, MÓ TRAMPO
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
...
def tela_derrota():
    largura, altura = 800, 500
    tela = pygame.display.set_mode((largura, altura))
    pygame.display.set_caption("TELA DERROTA")
    img = pygame.image.load("img/DERROTA.png")#Define o fundo da imagem inicial
    botao = pygame.image.load("img/botao.png")
    botao = pygame.transform.scale(botao, (240, 80))
    botao_rect = botao.get_rect()
    botao_rect.centerx = largura // 2
...
def atacar(mago, magia, mago_atacando):
    dano = magia.poder
    if mago.energia <=0:
        mago.vivo = False
        ...
        if mago.nome == "player":
            ...
            tela_vitoria()
    if defesa(mago):
        if mago_atacando.energia < magia.custo_energia:
            print(f"{mago_atacando.nome} não possui energia suficiente para lançar essa magia!")
            mago_atacando.vivo = False
            if mago_atacando.energia <=0:
                mago_atacando.vivo = False
            print(f'{mago_atacando.nome} morreu, mago {mago.nome} é o vencendor!')
            if mago_atacando.nome == "player":
                som(mago_atacando)
                tela_derrota()
    ...
    mago_atacando.energia -= magia.custo_energia
    ...
    if mago.energia <= 0:
        ...
        print(f"{mago.nome} morreu")
        telainicial()
    if mago.energia <=0 and mago_atacando.energia <=0:
        print("Deu empate")
        telainicial()
...
def som(mago):
    if mago.nome == "player":
        pygame.mixer.init()
        som_derrota = pygame.mixer.Sound("sons/efeito_derrota.mpeg")
        som_derrota.play()

def desenhar_imagem(mago):
    largura, altura = 800, 500
    tela = pygame.display.set_mode((largura, altura))
    ...
    if mago.nome == "player":
        ...
        imagem = pygame.image.load("img/DERROTA.png")
    ...
    while True:
                    ...
                    if mage_player.vivo:
                        for magia in magias_player:
                            if magia.rect.collidepoint(event.pos):
                                ...
                                danoplayer = atacar(mago_inimigo, magia, mage_player)
        ...
        if turno_mago_inimigo and mago_inimigo.vivo:
            ...
            if tempo_atual - ultimo_turno >= intervalo_turnos:
                ...
                danoinimigo = atacar(mage_player, magia_inimigo, mago_inimigo)
