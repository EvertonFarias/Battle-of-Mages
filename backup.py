...
def telamagos(): #responsável por rodar o loop da tela de escolha do mago
    global escolha
    ...
    botao_rect4 = botaomagoterra.get_rect()
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                quit()
            if event.type == pygame.MOUSEBUTTONDOWN:
                if botao_rect.collidepoint(event.pos):#rect do mago de fogo            
                     escolha = 'Fogo'
                     telaescolhaarena()
            if event.type == pygame.MOUSEBUTTONDOWN:
                if botao_rect2.collidepoint(event.pos):#rect do mago de agua
                    escolha = 'Agua'
                    telaescolhaarena()
            if event.type == pygame.MOUSEBUTTONDOWN:
                if botao_rect3.collidepoint(event.pos):#rect do mago de terra
                    escolha = 'Terra'
                    telaescolhaarena()
            if event.type == pygame.MOUSEBUTTONDOWN:#rect do mago de ar
...
def atacar(mago, magia, tela):
    if magia.poder >= mago.vida:
        mago.vida = 0
    else:
        mago.vida -= magia.poder
    ...
    while True:
                    ...
                    if mage_player.vivo:
                        for magia in magias_player:
                            if magia.rect.collidepoint(event.pos):
                                ...
                                atacar(mago_inimigo, magia, tela5)
                                danos_jogador.append((magia.poder, 560, 300, pygame.time.get_ticks()))  # Adiciona o dano à lista
        ...
        if turno_mago_inimigo and mago_inimigo.vivo:
            ...
            if tempo_atual - ultimo_turno >= intervalo_turnos:
                ...
                atacar(mage_player, magia_inimigo, tela5)
                danos_inimigo.append((magia_inimigo.poder, 20, 300, pygame.time.get_ticks()))  # Adiciona o dano à lista
